...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const embed = competition('match', isHome, enemy, null);
    embed.setImage(getMatchGif(isHome));
    embed.setThumbnail(logoUrl);
    const target = salonOpt || interaction.channel;
    await target.send({ content: '@everyone', embeds: [embed] });
    await interaction.editReply({ embeds: [{ color: 0xFF6BB5, description: `✅ Match posté dans <#${target.id}> !` }] });
  }
};
