...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand(s => s.setName('lien').setDescription('Logs des tentatives de liens bloqués').addChannelOption(o => o.setName('salon').setDescription('Salon de logs liens').addChannelTypes(ChannelType.GuildText).setRequired(true))),

  async execute(interaction) {
    if (!await requireAdmin(interaction)) return;
    const sub = interaction.options.getSubcommand();
    const ch  = interaction.options.getChannel('salon');
    const keys = { vocal: 'log_vocal', message: 'log_message', tickets: 'log_tickets', moderation: 'log_moderation', lien: 'log_lien' };
    const icons = { vocal: '🔊', message: '💬', tickets: '🎫', moderation: '🔨', lien: '🔗' };
...
// Note: /logs lien est géré dans ce fichier via le subcommand ci-dessous
